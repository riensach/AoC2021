...
foreach($inputArray as $key => $value) {
    ...
    // Check if only one point needs flipping, aka and reverse diagonal line
    if($lineStart[1] > $lineEnd[1] XOR $lineStart[0] > $lineEnd[0]) {
        if($lineStart[1] > $lineEnd[1]) {
            ...
            while($lineStartColumnIterator <= $lineEnd[0]) {
                while($lineStartRowIterator >= $lineEnd[1]) {
                    ...
                    //echo "&nbsp;&nbsp;&nbsp;&nbsp;Placing line at $lineStartColumnIterator,$lineStartRowIterator (special 1)<br>";
                    ...
                    if($lineStartColumnIterator <> $lineEnd[0] && $lineStartRowIterator <> $lineEnd[1]) {
            ...
            while($lineStartColumnIterator >= $lineEnd[0]) {
                while($lineStartRowIterator <= $lineEnd[1]) {
                    ...
                    $currentGridArrayValue = $gridArray[$lineStartRowIterator][$lineStartColumnIterator];
                    //echo "&nbsp;&nbsp;&nbsp;&nbsp;Placing line at $lineStartColumnIterator,$lineStartRowIterator (special 2)<br>";
                    if($currentGridArrayValue=='.') {
                        $gridArray[$lineStartRowIterator][$lineStartColumnIterator] = 1;
                    } else {
                        $gridArray[$lineStartRowIterator][$lineStartColumnIterator] = $gridArray[$lineStartRowIterator][$lineStartColumnIterator] + 1;
